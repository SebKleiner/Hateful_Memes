...
@app.route('/')
def hello():
    return "Welcome to prediction of hateful memes"
...
def predict_single():
    ...
    """
    labels=['Gun', 'Shooting sport', 'Airsoft', 'Shooting', 'Recreation', 'Airsoft gun', 'Air gun', 'Games']
    objects={'Person'}
    text="what's a word that begins with n, ends in r and you never want to call a black person? a neighbour"

    input = vectorize(labels, objects, text)
    probas = get_probabilities(input)
    pred = get_prediction(probas)

    if pred==1:
        return 'This is a heteful meme'
    else:
        return 'This is a safe meme'
